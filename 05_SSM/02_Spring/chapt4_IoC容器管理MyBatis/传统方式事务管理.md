...
- [目录](#目录)
- [1. Mybatis事务管理](#1-mybatis事务管理)
  - [1.1 事务的ACID原则](#11-事务的acid原则)
  - [1.2 事务的隔离级别与并发问题](#12-事务的隔离级别与并发问题)
  - [1.3 Mybatis事务底层源码浅析](#13-mybatis事务底层源码浅析)
...
- 原子性（Atomicity）：事务是一个原子操作，由一系列动作组成。事务的原子性确保**动作要么全部完成，要么完全不起作用。**
...
简单来说，事务就是 **要么完成，要么就啥都别做!** 并且不同的事务之间相互隔离，互不干扰。
