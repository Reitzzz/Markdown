...
- [1. 用户和用户组](#1-用户和用户组)
  - [目录](#目录)
  - [1.1 用户机制与状态信息](#11-用户机制与状态信息)
  - [1.2 root超级用户与权限切换](#12-root超级用户与权限切换)
  - [1.3 用户的创建与删除](#13-用户的创建与删除)
  - [1.4 用户密码设置与登录切换](#14-用户密码设置与登录切换)
  - [1.5 用户组管理与权限赋予](#15-用户组管理与权限赋予)
  - [1.6 用户与组的配置文件](#16-用户与组的配置文件)
...
其中`study`就是我们想要创建的新用户，`useradd`命令就是创建新用户的命令
...
比如我们**现在需要在用户创建时顺便创建用户的文件夹，并指定shell**（任意一种命令解释程序，用于处理我们输入的命令）为bash：
