...
```java
@Configuration
@ComponentScan("org.example.springbootdemo") //开启包扫描 自动注册路径下的Bean属性
@PropertySource("classpath:test.properties",encoding="UTF-8")   //注意，类路径下的文件名称需要在前面加上classpath:
...
```java
@Component //表示是一个被 Spring 管理的 Bean 
public class Student {
    @Value("${test.name}")   //这里需要在外层套上 ${ }
                             //导入的包应该是import org.springframework.beans.factory.annotation.Value;
...
如果遇到乱码的情况，请将配置文件的编码格式切换成UTF-8（可以在IDEA设置中进行配置）然后在 `@PropertySource` 注解中添加属性 `encoding = "UTF-8"` 这样就正常了，当然，其实一般情况下也很少会在配置文件中用到中文。
