...
**目录**
- [1. 关联查询](#1-关联查询)
  - [1.1 一对一关联配置](#11-一对一关联配置)
  - [1.2 懒加载 (Lazy Loading)](#12-懒加载-lazy-loading)
  - [1.3 级联关联操作 (Cascading)](#13-级联关联操作-cascading)
  - [1.4 一对多关联 (One-to-Many)](#14-一对多关联-one-to-many)
  - [1.5 多对一关联 (Many-to-One)](#15-多对一关联-many-to-one)
  - [1.6 多对多关联 (Many-to-Many)](#16-多对多关联-many-to-many)
...
public class Account {
    ...
    /* 它告诉 JPA：“在 account 这张表里，有一个叫 detail_id 的外键字段。
    请你拿着这个 detail_id 的值，去 account_detail 表（对应 AccountDetail 实体）的主键里匹配，
    从而帮我把完整的数据查出来并封装到下面的 detail 对象中。” */

    @OneToOne(cascade = CascadeType.ALL)   //声明为一对一关系
                        //CascadeType.ALL 就像是一个“全自动同步开关”：只要你动了父对象，子对象也会跟着动。

    AccountDetail detail;  
    
    /*表示当前 Account 对象拥有一个与之关联的 AccountDetail 对象
    你在 Java 代码里查询出一个 Account 实体时，JPA 会自动帮你把对应的 AccountDetail 实体也查出来
    并赋值给这个 detail 变量 */
}
```
