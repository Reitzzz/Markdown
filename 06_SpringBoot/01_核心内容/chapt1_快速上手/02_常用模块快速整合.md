...
## 目录
- [1. 常用模块快速整合](#1-常用模块快速整合)
  - [目录](#目录)
  - [1.1 Spring Boot Starter 基础](#11-spring-boot-starter-基础)
  - [1.2 整合 Spring Web](#12-整合-spring-web)
    - [手动添加WebMvc的情况](#手动添加webmvc的情况)
  - [1.3 整合 Spring Security](#13-整合-spring-security)
  - [1.4 整合 Thymeleaf 模板引擎](#14-整合-thymeleaf-模板引擎)
  - [1.5 整合 MyBatis](#15-整合-mybatis)

---

## 1.1 Spring Boot Starter 基础

前面我们说了，SpringBoot的核心思想就是**约定大于配置**，能在一开始默认的就直接默认，不用我们自己来进行配置，我们只需要配置某些特殊的部分即可，这一部分我们就来详细体验一下。
...
### 手动添加WebMvc的情况
 ...
 1. 注册拦截器 <br>
当你需要对请求进行预处理或后处理（例如：登录状态校验、接口防刷、请求日志记录）时，需要实现 `HandlerInterceptor` 接口，并通过配置类的 `addInterceptors` 方法将其注册到 Spring MVC 中，同时配置需要拦截或放行的 URL 路径。

 2. 配置全局跨域 <br>
虽然可以在 Controller 或方法上使用 `@CrossOrigin` 注解实现局部跨域，但如果需要为整个项目配置统一的跨域策略（允许的请求域名、方法、请求头等），通常会在配置类中重写 `addCorsMappings` 方法来进行全局配置。

 3. 自定义静态资源映射 <br>
Spring Boot 默认将静态资源映射到 `classpath:/static/` 或 `classpath:/public/` 等目录。如果你需要以下操作，就需要重写 `addResourceHandlers` 方法：
* 将服务器本地硬盘上的目录（如用户上传的图片文件夹）映射为可通过 HTTP 访问的 Web 资源。
* 自定义前端访问静态资源的 URL 路径前缀。

 4. 自定义或替换消息转换器 <br>
Spring Boot 默认使用 Jackson 处理 JSON 数据。如果在项目中需要：
* 将默认的 JSON 库替换为 FastJson 或 Gson。
* 全局修改序列化规则（例如：将 Long 类型转为 String 传给前端以防止精度丢失，或配置全局的日期时间格式化）。
此时需要重写 `configureMessageConverters` 或 `extendMessageConverters` 方法。


 5. 添加自定义参数解析器 <br>
当控制器的方法参数需要注入通过特定逻辑获取的对象时（例如：自定义一个 `@CurrentUser` 注解，直接在 Controller 参数中获取当前登录的用户信息），需要编写实现 `HandlerMethodArgumentResolver` 的解析器，并在配置类中通过 `addArgumentResolvers` 注册。

 6. 配置简单的页面跳转 <br>
对于一些不需要任何业务处理，只需要将特定 URL 直接路由到某个视图模板或另一个 URL 的情况，可以通过重写 `addViewControllers` 方法来直接建立映射。这样可以省去编写空的 Controller 方法的麻烦。

 7. 自定义数据类型转换器 <br>
当需要将前端传来的非标准格式字符串自动转换为后端的 Java 对象时（例如，将特定格式的字符串转换为 Date 或 LocalDateTime 对象，或者将字符串映射为自定义枚举），可以通过重写 `addFormatters` 方法注册自定义的 Converter。
