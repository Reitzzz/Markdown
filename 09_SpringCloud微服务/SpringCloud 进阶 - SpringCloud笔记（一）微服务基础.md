...
<dependency>
    ...
    <!-- 对于SpringBoot 3.x，需使用2022.0.x或2023.0.x之后的版本 -->
...
```xml
<!-- SpringBoot 3.x 已彻底废弃 Hystrix，改为使用 Spring Cloud CircuitBreaker (Resilience4j) -->
   <dependency>
        <groupId>org.springframework.cloud</groupId>
...
```java
@SpringBootApplication
// @EnableHystrix   // SpringBoot 3.x 搭配 Resilience4j 时，无需该注解即可启用熔断机制
public class BorrowApplication {
    public static void main(String[] args) {
        SpringApplication.run(BorrowApplication.class, args);
    }
...
public class BorrowController {
    ...
    @io.github.resilience4j.circuitbreaker.annotation.CircuitBreaker(name = "borrow", fallbackMethod = "onError")    // SpringBoot 3.x 使用@CircuitBreaker来指定备选方案
    @RequestMapping("/borrow/{uid}")
    UserBorrowDetail findUserBorrows(@PathVariable("uid") int uid){
        return service.getUserBorrowDetailByUid(uid);
    }
  	...
  	//备选方案，这里直接返回空列表了
  	//注意对于 Resilience4j，备选方案的参数需要比实际方法多接收一个 Throwable 异常参数
...
```xml
<!-- SpringBoot 3.x 已经无官方支持的 Hystrix Dashboard，通常改用 Prometheus + Grafana 等监控方案。以下依赖在 Spring Boot 3 中不再适用，仅保留原文供旧版参考 -->
<dependency>
    <groupId>org.springframework.cloud</groupId>
...
原文链接：https://www.itbaima.cn/zh-CN/document/oejzo0l77zeb6a7e